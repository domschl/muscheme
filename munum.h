...
struct munum {
    ...
    munum muipsub(munum num1, munum num2) {
        ...
        while (rp.nom[0]=='0' && rp.nom.length()>1) rp.nom=rp.nom.substr(1);
        return rp;
    }
    ...
    munum mufac(munum n) {
        ...
        if (mueq(n,munum(0))) return munum(1);
        while (true) {
            f=mumul(f,n);
            n=musub(n,munum(1));
   ...
   bool mugt(munum num1, munum num2) {
    ...
    munum mugcd(munum num1, munum num2) {
        munum r(0);
        if (num1.den!="1" || num2.den!="1" || !num1.pos || !num2.pos || num1.type!=munum::mum_valid || num2.type!=munum::mum_valid) {
            r.to_nan();
            return r;
        }
        munum t;
   ...
   munum mufactor(munum num1) {
    ...
    std::vector<munum> muipdivmod(munum num1, munum num2) {
        munum r(0),q(0);
        int dl;
    ...
    std::vector<munum> mudivmod(munum num1, munum num2) {
        ...
        if (s1!=s2) {
            if (res[0].nom!="0") res[0].pos=false;
        }
        if (!s1) {
            res[1].pos=false;
        }
